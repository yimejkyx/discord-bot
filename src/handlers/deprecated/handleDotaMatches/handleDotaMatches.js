...
const { fetchDotaMatches } = require("./helpers/fetchDotaMatches");

let matches = fetchDotaMatches();

function getMatchString(match) {
  return `${match.teamLeft} VS ${
    match.teamRight
  ...
  new CronJob("1 */20 * * * *", async () => {
    ...
    if (parsedMatches) {
      ...
      const channels = client.channels.cache.filter(
        (obj) => obj.type === "text" && obj.id === mainJokerickChannelID
      );
      ...
      parsedMatches.forEach((match) => {
        ...
        if (diff > 0 && diff < ONE_HOUR) {
          ...
          channels.forEach((channel) => {
            channel.send(
              `@everyone **Dotka soon retards** ${getMatchString(match)}\n${
                match.url
              } in ${fixedTime} minutes`
            );
          });
        }
      });
...
async function handleDotaMatches(client, msg) {
  ...
  if (member) {
    const isCommand =
      content === `${cmdPrefix}dota` ||
      content === `${cmdPrefix}dotka` ||
      content === `${cmdPrefix}nip`;
    if (isCommand) {
      const { channel } = msg;
      if (channel) {
