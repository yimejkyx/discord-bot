...
export async function playRequest(msg, requestText, voice) {
  ...
  try {
    ...
    if (!songInfo) {
      reply = await msg.reply(
        "Sry, cannot find video, nieco sa doondialo :((((("
      );
    ...
    if (songInfo.videoLength > 0) {
      reply = await msg.reply(
        `playing '${songInfo.title}' for ${songInfo.videoLengthMs / 1000}s`
      );
    } else {
    ...
    reply = await msg.reply(
      "Sry, cannot play that video, nieco sa doondialo :((((("
    );
  }
