...
class VoiceManager {
    ...
    static play(songInfo) {
        ...
        if (songInfo.videoLength > 0) {
            logger.debug(`VoiceManager.play: setting up clean, will clean after ${songInfo.videoLengthMs}ms`);
            this.state.stoppingTimeout = setTimeout(() => this.timeoutFn(songInfo.title), songInfo.videoLengthMs + STOP_SECONDS_OFFSET);
        } else {
