...
const { defiPulseApiKey, cmdPrefix } = config;

export async function handleGasPriceCron(client, gasState) {
  ...
  new CronJob("1 */10 * * * *", async () => {
    try {
      ...
      const res = await axios.get(
        `https://ethgasstation.info/api/ethgasAPI.json?api-key=${defiPulseApiKey}`
      );
      const { data } = res;
      ...
      gasState.listeners.forEach((listener) => {
        ...
        const guild = client.guilds.cache.find(
          (guild) => guild.id === listener.guildId
        );
        if (!guild) return;

        const user = guild.members.cache.find(
          (user) => user.id === listener.userId
        );
        if (!user) return;

        const channel = guild.channels.cache.find(
          (channel) => channel.id === listener.channelId
        );
        if (!channel) return;

        channel.send(
          `<@${user.id}> ETH limit avg < ${listener.limit}: fastest **${
            fastest / 10
          }** (<30s), fast **${fast / 10}** (<2m), average **${
            average / 10
          }** (<5m), safe low **${safeLow / 10}** (<30m)`
        );
      });
    } catch (err) {
...
export async function handleGasPrice(client, msg, gasState) {
  ...
  const hasListener = gasState.listeners.find(
    (listener) => listener.userId === member.id
  );
  if (hasListener) {
    // removing listener
    logger.info(`Removing user "${member.nickname}" listener`);
    gasState.listeners = gasState.listeners.filter(
      (listener) => listener.userId !== member.id
    );

    const reply = await msg.reply("Removing your gas price watcher");
...
export async function handleGasChart(client, msg, gasState) {
  const { content, member } = msg;
  if (!member) return;
  ...
  const values = gasState.prices.reduce(
    (arr, item) => [
      ...arr,
