...
const { cmdPrefix, canDeleteMessages } = config;

let isDeleting = false;

export async function handleDeletingLastMessages(client, msg) {
  const { content, member } = msg;
  if (!member) return;

  const roles = member.roles.cache.map(({ name }) => name);
  const hasRole = canDeleteMessages.some((role) => roles.includes(role));
  const isCommand =
  ...
  if (!isDeleting && hasRole && isCommand) {
    ...
    if (channel) {
      ...
      try {
        ...
        const errorReply = await msg.reply(
          "Sry, nieco sa doondialo pri mazani"
        );
